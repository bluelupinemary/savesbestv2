...
<div style="margin-left:5%;" class="container">
      <br/><br/>
      <p>This page will allow you to import consumers' readings given the chosen file.</p>
      <br/>
      <?php
        if(!$ok){
          echo '<h4>Please click <i><span style="color:maroon;">CHOOSE FILE</span></i> button first and then <i><span style="color:maroon;">IMPORT READINGS</span></i> button.</h4>';
        }
        ?>
    ...
    <div>
      <?php 
        if(!$ok){
            ...
            echo "<input class='btn btn-success' type='submit' name='submit' value='Import Readings' />";
        }
      ?>
      </form>
      <?php
        if($ok){
            echo form_open('Consumer/showImportedReadings');
            //echo "<input type='hidden' name='reading_id' id='reading_id' value='".$result['id']."' />";
            echo '<button class="btn btn-warning" type="submit">Show Imported Readings</a>';
